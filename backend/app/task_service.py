...
FLOW_STEPS = [
    ...
    {"step": 4, "name": "人力计算", "type": "design", "roles": ["tse"]},
    {"step": 5, "name": "环境确认", "type": "execute", "roles": ["version"]},
    {"step": 6, "name": "环境审核", "type": "review", "roles": ["pl"]},
    {"step": 7, "name": "执行测试", "type": "execute", "roles": ["executor"]},
    {"step": 8, "name": "问题记录", "type": "execute", "roles": ["executor", "tse", "version", "pl"]},
    {"step": 9, "name": "测试完成", "type": "execute", "roles": ["executor"]},
    {"step": 10, "name": "数据审核", "type": "review", "roles": ["version", "tse"]},
    {"step": 11, "name": "任务结束", "type": "end", "roles": ["pl"]},
]
...
class TaskService:
    ...
    async def _init_steps(self, task_id: int, task_data: TaskCreate):
        ...
        owner_map = {
            ...
            4: task_data.tse_id,
            5: task_data.version_owner,
            6: task_data.owner,
            7: None,  # 测试执行，多人
            8: None,  # 全员
            9: None,  # 测试执行
            10: task_data.version_owner,
            11: task_data.owner,
        }
    ...
    async def get_tasks(
        ...
        """获取任务列表"""
        stmt = select(TestTask).where(TestTask.deleted == False)
    ...
    async def update_step(
        ...
        # 更新任务进度
        # 业务规则：环节6(环境审核)打回 → 环节5(环境确认)退回重做
        if step.step == 6 and data.status == "rejected":
            await self._reset_step(step.task_id, 5)

        # 业务规则：环节10(数据审核)不通过需补测 → 环节6~9全部退回，
        # 且环节6处理人从PL改为版本负责人（由版本负责人牵头补测）
        if step.step == 10 and data.status == "rejected":
            ...
            if task and task.version_owner:
                stmt6 = select(TaskStep).where(
                    TaskStep.task_id == step.task_id, TaskStep.step == 6
                )
                r6 = await self.db.execute(stmt6)
                step6 = r6.scalar_one_or_none()
                if step6:
                    step6.assigned_to = task.version_owner
    ...
    def _get_status_by_step(self, step: int) -> str:
        ...
        mapping = {
            ...
            4: TaskStatus.design.value,
            5: TaskStatus.env_confirm.value,
            6: TaskStatus.env_approved.value,
            7: TaskStatus.testing.value,
            8: TaskStatus.testing.value,
            9: TaskStatus.testing.value,
            10: TaskStatus.review.value,
            11: TaskStatus.done.value,
        }
